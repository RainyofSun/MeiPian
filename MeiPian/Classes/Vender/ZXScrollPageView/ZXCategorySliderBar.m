...
- (void)setSelectIndex:(NSInteger)index{
    ...
    [self adjustContentOffsetToIndex:index completeHanle:^{
        ...
        itemView.contentLabel.textColor = HexColor(0xFF5532);
        NSNotification *notification = [[NSNotification alloc]initWithName:RESETCOLORNOTIFICATION object:@"color" userInfo:@{@"color":HexColor(0x939DA6),@"index":@(self.currentIndex)}];
...
- (UIView *)indicateView
{
    if (!_indicateView) {
        ...
        _indicateView.backgroundColor = HexColor(0xFF5532);
    }
                  ...
                  cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row == self.currentIndex) {
        cell.contentLabel.textColor = HexColor(0xFF5532);
    }
    else{
        cell.contentLabel.textColor = HexColor(0x939DA6);
    }
    return cell;
}
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSNotification *notification = [[NSNotification alloc]initWithName:RESETCOLORNOTIFICATION object:@"color" userInfo:@{@"color":HexColor(0x939DA6),@"index":@(indexPath.row)}];
    [[NSNotificationCenter defaultCenter]postNotification:notification];
...
- (void)adjustContentOffsetToIndex:(NSInteger)index completeHanle:(void(^)(void))completeHandle animation:(BOOL)animation{
...
- (void)scrollToPoint:(CGPoint)point CompleteHandle:(void(^)(void))completeHandle animation:(BOOL)animation{
    if (animation) {
