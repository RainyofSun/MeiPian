...
- (NSLayoutConstraint *)equallyRelatedConstraintWithView:(UIView *)view attribute:(NSLayoutAttribute)attribute multiplier:(CGFloat)ratio;
...
- (NSInteger)dzn_itemsCount
{
    ...
    if (![self respondsToSelector:@selector(hourSource)]) {
        return items;
    }
...
- (UIColor *)dzn_dataSetBorderColor
{
    if (self.emptyDataSetSource && [self.emptyDataSetSource respondsToSelector:@selector(borderColorForEmptyDataSet:)]) {
        UIColor *color = [self.emptyDataSetSource borderColorForEmptyDataSet:self];
        if (color) NSAssert([color isKindOfClass:[UIColor class]], @"You must return a valid UIColor object for -backgroundColorForEmptyDataSet:");
        return color;
    }
    return [UIColor clearColor];
...
- (void)dzn_reloadEmptyDataSet
{
    ...
    if (([self dzn_shouldDisplay] && [self dzn_itemsCount] == 0) || [self dzn_shouldBeForcedToDisplay])
    {
        ...
        view.button.layer.borderColor = [self dzn_dataSetBorderColor].CGColor;
        view.hidden = NO;
...
- (UIButton *)button
{
    if (!_button)
    {
        ...
        _button.layer.borderColor = [UIColor colorWithRed:255/255.0 green:102/255.0 blue:48/255.0 alpha:1].CGColor;
...
- (void)setupConstraints
{
    ...
    NSLayoutConstraint *centerYConstraint = [self equallyRelatedConstraintWithView:self.contentView attribute:NSLayoutAttributeCenterY multiplier:0.75];
    ...
    if (_customView) {
        ...
        // TODO  修改库代码
        CGFloat buttonPadding = (CGRectGetWidth([UIScreen mainScreen].bounds) - 122)/2;
        NSDictionary *button_metrics = @{@"btnPadding":@(buttonPadding)};
        ...
        if ([self canShowButton]) {
            ...
            //            [self.contentView addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"H:|-(padding@750)-[button(122)]-(padding@750)-|"
            //                                                                                     options:0 metrics:metrics views:views]];
            
            // TODO 修改库代码
            [self.contentView addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"H:|-(btnPadding@750)-[button(122)]-(btnPadding@750)-|"
                                                                                     options:0 metrics:button_metrics views:views]];
        }
        // or removes from its superview
        else {
