...
- (void)layoutSubviews {
    ...
    if (self.rightButton) {
        ...
        collectionW -= (btnW + self.config.btnDistanceToEdge);
        self.rightButton.frame = CGRectMake(self.bounds.size.width - btnW - self.config.btnDistanceToEdge, 0, btnW, btnW);
    }
    if (self.leftButton) {
        CGFloat btnW = self.bounds.size.height;
        self.leftButton.frame = CGRectMake(self.config.btnDistanceToEdge, 0, btnW, btnW);
        collectionX += (btnW + self.config.btnDistanceToEdge);
        collectionW -= (btnW + self.config.btnDistanceToEdge);
    }
    self.collectionView.frame = CGRectMake(collectionX, 0, collectionW, self.bounds.size.height);
...
- (CGPoint)shadowLineCenterForIndex:(NSInteger)index {
    ...
    if (self.config.shadowLineAlignment == XLPageShadowLineAlignmentTop) {
        centerY = self.config.shadowLineHeight/2.0f + self.config.shadowLineDistanceToEdge;
    }
    if (self.config.shadowLineAlignment == XLPageShadowLineAlignmentCenter) {
        centerY = CGRectGetMidY(cellFrame);
    }
    if (self.config.shadowLineAlignment == XLPageShadowLineAlignmentBottom) {
        centerY -= self.config.shadowLineDistanceToEdge;
    }
