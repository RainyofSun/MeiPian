...
@interface MPCirclePageView ()<UIScrollViewDelegate,UIGestureRecognizerDelegate>
...
- (void)layoutSubviews {
    ...
    self.interestView.frame = CGRectMake(0, self.recommandView.recommandH, CGRectGetWidth(self.bounds), ScreenHeight);
}
...
- (void)setupUI {
    ...
    self.mainScrollView.contentSize = CGSizeMake(0, ScreenHeight + self.recommandView.recommandH);
...
- (void)controlScrollForVertical:(CGFloat)detal AndState:(UIGestureRecognizerState)state {
    ...
    if (self.mainScrollView.contentOffset.y >= self.recommandView.recommandH) {
        ...
        if (offsetY < 0) {
            ...
            offsetY = self.interestView.listTableView.contentOffset.y - rubberBandDistance(detal, ScreenHeight);
        }
        ...
        if (mainOffsetY < 0) {
            ...
            if (subScrollViewOffsetY < 0) {
                // 触发子Scrollview下拉刷新
                mainOffsetY = 0;
                subScrollViewOffsetY = self.interestView.listTableView.contentOffset.y - rubberBandDistance(detal, ScreenHeight);
